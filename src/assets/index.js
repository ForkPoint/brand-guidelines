import logo from "./logo.svg";
import brandingHero from "./branding-hero.jpg";
import fullNegative from "./full-negative.png";
import fullPositive from "./full-positive.png";
import manoNegative from "./mono-negative.png";
import manoPositive from "./mono-positive.png";
import forkpointGrandpa from "./forkpointGrandpa.png";
import logoType from "./constructionLogo.png";
import pencil from "./pencil.svg";
import webDraw from "./webArt.svg";
import cssPaint from "./cssPaint.svg";
import text from "./text.svg";
import logoDraw from "./logo.svg";
import forkpointVector1 from "./logoVector1.svg";
import forkpointVector2 from "./logoVector2.svg";
import forkpointVector3 from "./logoVector3.svg";
import forkpointVector4 from "./logoVector4.svg";
import forkpointClearspace from "./logoClearspaceVector.svg";
import forkpointIconClearspace from "./iconClearspace.svg";
import icon1 from "./align-center.svg";
import icon2 from "./align-justify.svg";
import icon3 from "./align-left.svg";
import icon4 from "./align-right.svg";
import icon5 from "./bold.svg";
import icon6 from "./calendar.svg";
import icon7 from "./cloud.svg";
import icon8 from "./code.svg";
import icon9 from "./coffee.svg";
import icon10 from "./copy.svg";
import icon11 from "./crop.svg";
import icon12 from "./edit.svg";
import icon13 from "./facebook.svg";
import icon14 from "./film.svg";
import icon15 from "./gift.svg";
import icon16 from "./globe.svg";
import icon17 from "./headphones.svg";
import icon18 from "./instagram.svg";
import icon19 from "./link.svg";
import icon20 from "./linkedin.svg";
import icon21 from "./mail.svg";
import icon22 from "./monitor.svg";
import icon23 from "./search.svg";
import icon24 from "./send.svg";
import icon25 from "./slack.svg";
import icon26 from "./trello.svg";
import icon27 from "./twitter.svg";
import icon28 from "./type.svg";
import icon29 from "./underline.svg";
import icon30 from "./youtube.svg";
import layout1 from "./layout1.jpg";
import layout2 from "./layout2.jpg";
import layout3 from "./layout3.jpg";
import layout4 from "./layout4.jpg";
import layout5 from "./layout5.jpg";
import layout6 from "./layout6.jpg";
import buttonLayouts from "./buttonSizes.svg";
import buttonEmptySpace from "./buttonPaddings.svg";
import square from "./square.svg";
import portrait from "./portrait.svg";
import landscape from "./landscape.svg";
import structureOne from "./structure1.svg";
import structureTwo from "./structure2.svg";
import structureThree from "./structure3.svg";
import structureFour from "./structure4.svg";
import structureFive from "./structure5.svg";
import structureSix from "./structure6.svg";
import photography1 from "./photographyAnnya.jpg";
import photography2 from "./photographyTeam.jpg";
import photography3 from "./photographyAnnyaPresent.jpg";
import photography4 from "./photographyMeeting.jpg";
import photography5 from "./photographyNewComputers.jpg";
import photography6 from "./photographyRelax.jpg";
import photography7 from "./photographyUniversity.jpg";
import doAndDont1 from "./compositionDo.jpg";
import doAndDont2 from "./compositionDont.jpg";
import doAndDont3 from "./packagesDo.jpg";
import doAndDont4 from "./packagesDont.jpg";
import doAndDont5 from "./tshirtsDo.jpg";
import doAndDont6 from "./tshirtsDont.jpg";
import doAndDont7 from "./partyDo.jpg";
import doAndDont8 from "./partyDont.jpg";
import doAndDont9 from "./summerDo.jpg";
import doAndDont10 from "./summerDont.jpg";
import socialMedia1 from "./instagramLayout.jpg";
import socialMedia2 from "./facebookLayout.jpg";
import socialMedia3 from "./websiteLayout.jpg";
import socialMedia4 from "./tiktokLayout.jpg";
import socialMedia5 from "./jobsLayout.jpg";
import socialMedia6 from "./linkedinLayout.jpg";

export const instagramLayout = socialMedia1;
export const facebookLayout = socialMedia2;
export const websiteLayout = socialMedia3;
export const tiktokLayout = socialMedia4;
export const jobsLayout = socialMedia5;
export const linkedinLayout = socialMedia6;
export const compositionDo = doAndDont1;
export const compositionDont = doAndDont2;
export const packagesDo = doAndDont3;
export const packagesDont = doAndDont4;
export const tshirtsDo = doAndDont5;
export const tshirtsDont = doAndDont6;
export const partyDo = doAndDont7;
export const partyDont = doAndDont8;
export const teamDo = doAndDont9;
export const teamDont = doAndDont10;
export const photographyPresentation = photography1;
export const photographyTeam = photography2;
export const photographyAnnyaPresent = photography3;
export const photographyMeeting = photography4;
export const photographyNewComputers = photography5;
export const photographyRelax = photography6;
export const photographyUniversity = photography7;
export const logoSVG = logo;
export const homePageHero = brandingHero;
export const fullNegativeLogo = fullNegative;
export const fullPositiveLogo = fullPositive;
export const MonoNegativeLogo = manoNegative;
export const MonoPositiveLogo = manoPositive;
export const grandpaForkpoint = forkpointGrandpa;
export const constructionLogo = logoType;
export const css = cssPaint;
export const pencilArt = pencil;
export const webArt = webDraw;
export const penArt = text;
export const logoArt = logoDraw;
export const logoVector1 = forkpointVector1;
export const logoVector2 = forkpointVector2;
export const logoVector3 = forkpointVector3;
export const logoVector4 = forkpointVector4;
export const logoClearspaceVector = forkpointClearspace;
export const iconClearspaceVector = forkpointIconClearspace;
export const alignCenter = icon1;
export const alignJustify = icon2;
export const alignLeft = icon3;
export const alignRight = icon4;
export const bold = icon5;
export const calendar = icon6;
export const cloud = icon7;
export const code = icon8;
export const coffee = icon9;
export const copy = icon10;
export const crop = icon11;
export const edit = icon12;
export const facebook = icon13;
export const film = icon14;
export const gift = icon15;
export const globe = icon16;
export const headphones = icon17;
export const instagram = icon18;
export const link = icon19;
export const linkedin = icon20;
export const mail = icon21;
export const monitor = icon22;
export const search = icon23;
export const send = icon24;
export const slack = icon25;
export const trello = icon26;
export const twitter = icon27;
export const type = icon28;
export const underline = icon29;
export const youtube = icon30;
export const gridLayout1 = layout1;
export const gridLayout2 = layout2;
export const gridLayout3 = layout3;
export const gridLayout4 = layout4;
export const gridLayout5 = layout5;
export const gridLayout6 = layout6;
export const buttonSizes = buttonLayouts;
export const buttonPaddings = buttonEmptySpace;
export const squareLayout = square;
export const portraitLayout = portrait;
export const landscapeLayout = landscape;
export const structure1 = structureOne;
export const structure2 = structureTwo;
export const structure3 = structureThree;
export const structure4 = structureFour;
export const structure5 = structureFive;
export const structure6 = structureSix;